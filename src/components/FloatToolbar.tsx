...
import { View, Pressable, Platform, StyleSheet, Text, useWindowDimensions, Alert } from "react-native";
...
export default function BottomFloatToolbar({style, afterPickCallBack}: FloatToolbarProps){
  ...
  const pickImage = async () => {
     if (Platform.OS === 'web') {
      Alert.alert("Error", t("Web platform doesn't support image pick function"))
      return
    }
    try {
      ...
      if (result.canceled)
        ...
        console.log(`The image pick assets result ${JSON.stringify(result.assets)}`)
        //@ts-ignore      
